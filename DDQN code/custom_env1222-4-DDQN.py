...
class DQNAgent:
    ...
    def save_model(self):
            self.model.save("gridworld-ddqn2.h5")
    ...
    def test(self):
        ...
        self.load("gridworld-dqn8000i10.h5")
        
        for i in range(1):
            state, reward, done, info = self.env.reset_for_test(8)
            state.resize(1, 132)
            done = False
            j = 0
            #while not done:
...
if __name__ == '__main__':
    ...
    ######state = env.reset()
"""
    for j in range(6):      # j = image id
        env.resetimage(j) 
        
        for i in range(1000):  # i = no. of time variation
            print ("i: ", i, "j: ", j)
            done = False
