...
class DQNAgent:
    ...
    def createModel(self):
        model = Sequential()
        model.add(Conv1D(filters=128, kernel_size=3, activation='relu', input_shape=(132, 1)))
    ...
    def replay(self):
        ...
        for sample in samples:
            ...
            my_value =  target_actionQvalue_listN[0][next_action] 
    ...
    def save_model(self):
            self.model.save("gridworld-dqnCC.h5")
...
class myGridWorld(object):
    ...
    def reset(self):
        ...
        Tsmart = round(random.uniform(0.1, 1.0), 1) #0.20 #(0~1) processing time
        Tedge = round(random.uniform(0.1, 1.0), 1) #0.98 #(0~1) processing time
...
if __name__ == '__main__':
    ...
    for j in range(2):      # j = image id
        env.resetimage(j) 
        
        for i in range(2000):  # i = no. of time variation
            print ("i: ", i, "j: ", j)
            done = False
